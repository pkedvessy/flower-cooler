...
class FlowerCooler
  ...
  def print_help()
    ...
    self.log("  FC simulate on|off|status")
    self.log("  FC simulate top|mid|bottom|hum <value>")
    self.log("  FC simulate door open|closed")
  ...
  def handle_console_line(line)
    ...
    if line == "simulate status"
      self.print_sim_status()
      return
    end
    if line == "simulate door open"
      self.set_sim_door("open")
      self.evaluate()
      self.log("simulation door updated")
      return
    end
    if line == "simulate door closed"
    ...
    if string.find(line, "simulate top ") == 0
      var v = self.to_num(string.replace(line, "simulate top ", ""))
      if v != nil
        self.sim_top = v
        self.evaluate()
        self.log("simulation top updated")
        return
      end
      self.log("usage: FC simulate top <degC>")
      return
    end
    if string.find(line, "simulate mid ") == 0
      var v = self.to_num(string.replace(line, "simulate mid ", ""))
      if v != nil
        self.sim_mid = v
        self.evaluate()
        self.log("simulation mid updated")
        return
      end
      self.log("usage: FC simulate mid <degC>")
      return
    end
    if string.find(line, "simulate bottom ") == 0
      var v = self.to_num(string.replace(line, "simulate bottom ", ""))
      if v != nil
        self.sim_bottom = v
        self.evaluate()
        self.log("simulation bottom updated")
        return
      end
      self.log("usage: FC simulate bottom <degC>")
      return
    end
    if string.find(line, "simulate hum ") == 0
      var v = self.to_num(string.replace(line, "simulate hum ", ""))
      if v != nil
        self.sim_hum = self.clamp(v, 0, 100)
        self.evaluate()
        self.log("simulation humidity updated")
        return
      end
      self.log("usage: FC simulate hum <%RH>")
      return
    end
